...
# Files outputted :
  # plot_log_prevented_cases.tiff : Diseases cases prevented per sex by walking in 2019 
  # HIA_log_walking_2019.xlsx : Cases, daly, costs (medical + social) prevented by walking in 2019
  # plot_log_deaths_prevented.tiff : Deaths prevented per sex by walking in 2019
  # plot_log_YLL_prevented.tiff : YLL prevented per sex by walking in 2019 
...
for (dis in dis_vec) {
  ...
  health_walkers <-  loglinear_reduction_risk(health_walkers, dis, params$rr_women, params$rr_men, params$ref_women, params$ref_men)
}
...
for (dis in dis_vec) {
  params <- dis_setting(dis)
  health_walkers_ub <-  loglinear_reduction_risk(health_walkers_ub, dis, params$rr_women_lb, params$rr_men_lb, params$ref_women, params$ref_men)
...
for (dis in dis_vec) {
  params <- dis_setting(dis)
  health_walkers_lb <-  loglinear_reduction_risk(health_walkers_lb, dis, params$rr_women_ub, params$rr_men_ub, params$ref_women, params$ref_men)
}
...
export(burden_IC, here("output", "Tables", "Log linear", "HIA_log_walking_2019.xlsx"))
...
#Export plot
ggsave(here("output", "Plots", "Log linear", "plot_log_cases_prevented.tiff"), plot = cases_prevented)
...
#Export plot
ggsave(here("output", "Plots", "Log linear", "plot_log_DALY_prevented.tiff"), plot = daly_prevented)
...
# Export plot
ggsave(here("output", "Plots", "Log linear", "plot_log_deaths_prevented.tiff"), plot = deaths_prevented)
...
ggsave(here("output", "Plots", "Log linear", "plot_log_YLL_prevented.tiff"), plot = YLL_prevented)
