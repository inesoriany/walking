...
for(dist in dist_vec){
  ...
  for (perc in perc_vec){
    ...
    for(i in 1:N) {
      ...
      for(dis in dis_vec){
      burden_i <- log_burden_prevented_replicate(sample_drivers, dis, N=1, NULL) %>%        # HIA outcomes for 1 disease
        mutate(run = i,
               percentage = perc,
               distance = dist)
...
for (dist in dist_vec) {
  for (perc in perc_vec) {
    ...
    soc_costs_IC <- as.data.frame(t(calc_replicate_IC(scenario, "tot_soc_costs"))) %>% 
      rename(soc_costs_low = "2.5%", soc_costs_mean = "50%", soc_costs_sup = "97.5%") %>% 
      mutate(distance = dist, percentage = perc)
...
export(global_shift_IC, here("output", "Tables", "Log linear", "Modal shift", "HIA_modal_shift_100replicate.xlsx"))
...
# Export plots
ggsave(here("output", "Plots", "Log linear", "Modal shift", "plot_modalshift_cases_prevented.png"), plot = global_shift_cases)
ggsave(here("output", "Plots", "Log linear", "Modal shift", "plot_modalshift_daly_prevented.png"), plot = global_shift_daly)
ggsave(here("output", "Plots", "Log linear", "Modal shift", "plot_modalshift_costs_saved.png"),plot = global_shift_costs)
ggsave(here("output", "Plots", "Log linear", "Modal shift", "plot_modalshift_soc_costs_saved.png"),plot = global_shift_soc_costs)
...
# Export plot
ggsave(here("output", "Plots", "Log linear", "Modal shift", "plot_modalshift_morbidity_prevented.png"), plot = morbidity_shift_cases)
...
# Export plot
ggsave(here("output", "Plots", "Log linear", "Modal shift", "plot_modalshift_mortality_prevented.png"), plot = mortality_shift_cases)
...
morbi_mortality_shift_cases
  ...
  # Export plot
ggsave(here("output", "Plots", "Log linear", "Modal shift", "plot_modalshift_morbi_mortality_prevented.png"), plot = morbi_mortality_shift_cases)
...
# Export heatmap
ggsave(here("output", "Plots", "Log linear", "Modal shift", "modalshift_CO2_emit.png"), plot = CO2_shift)
